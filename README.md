...
Latest completed report: **`outputs/decision-validation-2026-09-10`** (standard preset), updated **2026-09-10 20:29 UTC**.
...
| 5,742 | 256 | 1,469,952 | 423.7 s | 34.5 s | 27 |
...
python simulate.py --config config.py --dry-run

python simulate.py --config config.py --output outputs/my-run
...
The app has one main chart with every scenario's median trajectory. Remove options to remove the corresponding lines. Click a point to see its scenario choices, event timeline and retirement outlook. Display names are readable; serialized keys and IDs remain stable.
...
- **25 figure types**, saved as 300-dpi PNGs, with coverage and interpretation records.
